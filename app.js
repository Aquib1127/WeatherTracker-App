...
function conditionToClass(main, id, isNight = false) {
  ...
  if (main.includes("smoke") || main.includes("haze") || main.includes("mist"))
    return "smoke";
...
function clearAnimations() {
  ...
  while (weatherCanvas.querySelector(".sun"))
    weatherCanvas.removeChild(weatherCanvas.querySelector(".sun"));
  const animEls = weatherCanvas.querySelectorAll(
    ".cloud, .drop, .snowflake, .haze-layer, .stars"
  );
...
function renderAnimationsFor(conditionClass) {
  ...
  if (conditionClass === "sunny") {
    ...
    c1.className = "cloud small";
    weatherCanvas.appendChild(c1);
    const c2 = document.createElement("div");
    c2.className = "cloud med";
    weatherCanvas.appendChild(c2);
...
function updateUI(current, forecastRaw) {
  ...
  fetch(
    ...
    .then((aqiData) => {
      ...
      switch (aqiValue) {
        case 1:
          aqiEl.style.color = "#4CAF50";
          break; // Good - Green
        case 2:
          aqiEl.style.color = "#CDDC39";
          break; // Fair - Lime
        case 3:
          aqiEl.style.color = "#FFC107";
          break; // Moderate - Amber
        case 4:
          aqiEl.style.color = "#FF5722";
          break; // Poor - Orange
        case 5:
          aqiEl.style.color = "#F44336";
          break; // Very Poor - Red
        default:
          aqiEl.style.color = "#e8eeff";
          break; // Fallback
      }
    })
    .catch(() => {
  ...
  sunTimesEl.textContent = `${displayTimeFromUnix(
    current.sys.sunrise,
    tz
  )} / ${displayTimeFromUnix(current.sys.sunset, tz)}`;
  ...
  if (forecastRaw && forecastRaw.list) {
    ...
    days.forEach((dayKey) => {
      ...
      el.innerHTML = `<strong>${d.toLocaleDateString(undefined, {
        weekday: "short",
      })}</strong>
        <div>${Math.round(useCelsius ? avg : cToF(avg))}${
        useCelsius ? "°C" : "°F"
      }</div>
